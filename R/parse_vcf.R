...
parse_vcf_slim<-function(dir="example/",infile="example.vcf", outfile=NA,ncores=2, index.col=NA, nind=c(108,10,1), outseq=NA, type="ncd1"){
  require(glue)
  require(testthat)
  require(data.table)
  #
  tmp<-glue("{dir}{infile}")
  pref=gsub(".vcf","",tmp)
  inp<-fread(tmp, skip="##", header=T)
  setnames(inp, "#CHROM","CHR")
  if(is.na(index.col)){
  ...
  #
  if(type=="ncd1"){
    vcf_ncd1(x=inp,npop=2,out=glue("{outfile}ncd1.out"), nind=nind[c(1,2)])
  }  
